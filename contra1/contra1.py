...
FONDO_NIVEL = generar_fondo(NIVEL_ANCHO, SCREEN_HEIGHT)
...
def juego(modo_jugadores, jugador_forzado, vidas_extra=0):
    ...
    while corriendo:
        ...
        # ================= DIBUJAR FONDO =================
        # Dibujar porción visible del fondo grande
        # Calculamos sub-area: (camara_x, 0, SCREEN_WIDTH, SCREEN_HEIGHT)
        # Asegurar que no nos salimos del surface
        area_bg = pygame.Rect(camara_x, 0, SCREEN_WIDTH, SCREEN_HEIGHT)
        # Si el nivel es infinito o muy largo, mejor dibujar repetido o paralaje
        # Aquí usamos un fondo estático ancho (FONDO_NIVEL)
        if camara_x < FONDO_NIVEL.get_width() - SCREEN_WIDTH:
            screen.blit(FONDO_NIVEL, (0, 0), area_bg)
        else:
             # Caso borde final
             screen.blit(FONDO_NIVEL, (0, 0), (FONDO_NIVEL.get_width() - SCREEN_WIDTH, 0, SCREEN_WIDTH, SCREEN_HEIGHT))
        ...
        for p in plataformas:
            ...
            if p.right > camara_x and p.left < camara_x + SCREEN_WIDTH:
                ...
                pygame.draw.rect(screen, (0, 80, 200), (rect_pantalla.x, rect_pantalla.y + 10, rect_pantalla.width, rect_pantalla.height - 10))
                pygame.draw.rect(screen, (0, 150, 0), (rect_pantalla.x, rect_pantalla.y, rect_pantalla.width, 10))
